...
const init = @import("src/init.zig");
const wm = @import("src/wm.zig");
const whoami = @import("src/whoami.zig");
const os = @import("src/os.zig");
const kernel = @import("src/kernel.zig");
...
pub fn build(b: *std.Build) !void {
    ...
    const kernel_version = try kernel.getKernelVersion(allocator);
    const shell_bin = shell.getShell();
    options.addOption([]const u8, "username", username);
    options.addOption([]const u8, "hostname", hostname);
    options.addOption([]const u8, "wm", wm_name);
    options.addOption([]const u8, "init", init_name);
